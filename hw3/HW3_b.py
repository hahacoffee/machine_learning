...
class NeuralNetwork():
    def __init__(self):
        ...
        self.synaptic_weights3 = 2 * random.random((l3, 3)) -1
    ...
    def train(self, training_set_inputs, training_set_outputs, number_of_training_iterations):
        ...
        for iteration in range(number_of_training_iterations):
            ...
            del4 = (T_output - output)*self.__sigmoid_derivative(output)
...
if __name__ == "__main__":
    ...
    for i in range(len(test_set_x)):
        count += 1.0
        if np.argmax(neural_network.forward_pass(test_set_inputs[i,:])) == np.argmax(neural_network.forward_pass(test_set_outputs[i,:])):
            hit += 1.0
